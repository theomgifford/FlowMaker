...
class TetratriangleCrossNeg(Component):
    """
    ...
    neg_gap: gap between positive and negative marks
    ...
    def __init__(self,structure,startjunc=None,settings={}, cxns_names=_cxns_names):
        ...
        comp_key = 'TetratriangleCrossNeg'
        ...
        if self.center_ratio > 1 or self.center_ratio < 0 or rect_length < 0 or 2*wing+self.cross_width > self.size or 2*wing+self.cross_width + 2*self.neg_gap > self.size or rect_length < self.neg_gap:
            print("bad alignment mark parameters")
            return
        
        coords = startjunc.coords
        
        side=[  (coords[0]+self.cross_width/2+self.neg_gap,coords[1]+self.cross_width/2+self.neg_gap),
                (coords[0]+self.cross_width/2+self.neg_gap,coords[1]+self.cross_width/2+rect_length),
                (coords[0]+self.cross_width/2+wing+self.neg_gap,coords[1]+self.size/2),
                (coords[0]+self.size/2,coords[1]+self.cross_width/2+wing+self.neg_gap),
                (coords[0]+self.cross_width/2+rect_length,coords[1]+self.cross_width/2+self.neg_gap),
                (coords[0]+self.cross_width/2+self.neg_gap,coords[1]+self.cross_width/2+self.neg_gap),
                ]
