...
class TetratriangleNeg(Component):
    """
    The negative of TetratrianglePos. Still four isosceles triangles, but
    offset by 45 degrees, and with more space in the middle.
    
    size: side of a square occupied by alignment mark
    vertex_angle: vertex angle of each isosceles triangle in TetratrianglePos
    spacing: space between subsequent alignment marks
    center_ratio: ratio between distance from vertex to center and size/2
    ...
    def __init__(self,structure,startjunc=None,settings={}, cxns_names=_cxns_names):
        ...
        tri=[  (coords[0],coords[1]+self.center_ratio*self.size/2),
                (coords[0]-half_base,coords[1]+self.size/2),
                (coords[0]+half_base,coords[1]+self.size/2),
                (coords[0],coords[1]+self.center_ratio*self.size/2)
                ]
        
        tri1=rotate_pts(tri,startjunc.direction+45,coords)
        tri2=rotate_pts(tri,startjunc.direction+90+45,coords)
        tri3=rotate_pts(tri,startjunc.direction+180+45,coords)
        tri4=rotate_pts(tri,startjunc.direction+270+45,coords)
